...
export class PageMeasurer {
    ...
    private onRefChanged(newRef: HTMLDivElement) {
        ...
        this.textArea = newRef.getElementsByClassName("input")[0] as HTMLTextAreaElement;
        this.textAreaMagic = newRef.getElementsByClassName("input-magic")[0] as HTMLDivElement;
    ...
    public getLinePosition(line: number): Position {
        ...
        const child = this.textAreaMagic.children[clamp(line*2, 0, len-1)] as HTMLElement;

        const pos: Position = [child.offsetLeft + child.offsetWidth + 13 , child.offsetTop - 23];
        return pos;
    }
}
