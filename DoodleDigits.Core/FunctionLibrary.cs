...
namespace DoodleDigits.Core {
    public class FunctionData {
        ...
        public readonly Func<Value[], ExecutionContext, Value> Function;

        public readonly Range ParameterCount;

        public FunctionData(string name, Func<Value, ExecutionContext, Value> function) {
            Name = name;
            ParameterCount = 1..1;
            Function = (parameters, context) => function(parameters[0], context);
        }

        public FunctionData(string name, Func<Value, Value, ExecutionContext, Value> function) {
            Name = name;
            ParameterCount = 2..2;
            Function = (parameters, context) => function(parameters[0], parameters[1], context);
        }

        public FunctionData(string name, Range parameterCount, Func<Value[], ExecutionContext, Value> function) {
            Name = name;
